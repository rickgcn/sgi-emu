...
#[test]
fn synchronous_timer_projection_matches_the_full_register_snapshot() {
    let mut crime = crime();
    let origin = SimTime::new(1_000);
    let _ = crime
        .piu
        .write(registers::TIMER, 0x1020_3040, origin, TIMEBASE_HZ);
    crime.observe_time(origin);

    let timer = crime.synchronous_timer_projection().unwrap();
    let snapshot = crime.synchronous_read_snapshot().unwrap();
    for delivery_time in [origin, SimTime::new(1_001), SimTime::new(1_960)] {
        for (address, length) in [
            ...
            assert_eq!(
                timer.read(address, length, delivery_time),
                snapshot.read(address, length, delivery_time)
            );
        }
    }
}
...
#[test]
fn synchronous_timer_write_matches_the_routed_register_access() {
    let request = CrimeSysAdRequest {
        id: CrimeTransactionId::new(7),
        time: SimTime::new(12_345),
        address: registers::TIMER,
        transfer: CrimeTransfer::write(
            0x0000_0000_1020_3040_u64.to_be_bytes().into(),
            CrimeByteEnable::enabled(8),
        ),
    };
    let mut routed = crime();
    let mut direct = routed.clone();

    routed.accept(request.clone()).unwrap();
    let routed_completion = next_non_trace(&mut routed);
    let direct_completion =
        CrimeAction::CompleteSysAd(direct.commit_synchronous_timer_write(&request));

    assert_eq!(direct_completion, routed_completion);
    assert_eq!(direct.save_state(), routed.save_state());
}

#[test]
fn synchronous_timer_projection_commit_matches_the_routed_register_access() {
    let write_time = SimTime::new(12_345);
    let request = CrimeSysAdRequest {
        id: CrimeTransactionId::new(7),
        time: write_time,
        address: registers::TIMER,
    ...
    routed.accept(request).unwrap();
    let _ = next_non_trace(&mut routed);
    let mut projection = direct.synchronous_timer_projection().unwrap();
    projection.base = 0x1020_3040;
    projection.base_time = write_time;
    assert!(direct.commit_synchronous_timer_projection(projection));
