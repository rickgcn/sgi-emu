...
/// Decision of an FPU conditional move: copy the source value or keep the
/// destination value.
///
/// The `MOVF.fmt`/`MOVT.fmt` instructions copy `fs` to `fd` when the selected
/// floating-point condition code matches their true/false selector, and the
...
pub enum Mips4Cp1MoveDecision {
    ...
    /// Keep the existing destination register value.
    KeepDestination,
}

impl Mips4Cp1MoveDecision {
    /// Decides `MOVF`/`MOVF.fmt`: move when the condition code is false.
    pub const fn move_conditional_false(fcc: bool) -> Self {
        if fcc {
            Self::KeepDestination
        } else {
            Self::Move
        }
    }

    /// Decides `MOVT`/`MOVT.fmt`: move when the condition code is true.
    pub const fn move_conditional_true(fcc: bool) -> Self {
        if fcc {
            Self::Move
        } else {
            Self::KeepDestination
    ...
    /// Decides `MOVN.fmt`: move when the GPR test value is not zero.
    pub const fn move_conditional_nonzero(gpr_rt: u64) -> Self {
        if gpr_rt != 0 {
            Self::Move
        } else {
            Self::KeepDestination
    ...
    /// Decides `MOVZ.fmt`: move when the GPR test value is zero.
    pub const fn move_conditional_zero(gpr_rt: u64) -> Self {
        if gpr_rt == 0 {
            Self::Move
        } else {
            Self::KeepDestination
    ...
    /// Returns whether the source value is copied into the destination register.
...
impl Mips4Cp1Fcsr {
    ...
    pub fn record_float_flags(&mut self, flags: FloatExceptionFlags) -> Result<(), Mips4Exception> {
        let decision = decide_float_exception(*self, flags);
        self.replace_cause(flags);

        if decision.traps {
            Err(Mips4Exception::FloatingPoint)
        } else {
            self.set_flag_flags(decision.flag_flags);
            Ok(())
        }
    }
...
/// Pure decision of a floating-point exception update in precise exception mode.
///
/// Mirrors the manual FPU exception model (MIPS IV manual section B.5.1). A trap
/// is generated when a cause bit and its matching enable bit are both set; there
/// is no enable for the Unimplemented Operation exception, which always traps.
/// In precise mode, instructions that trap do not update the sticky flag bits,
...
pub struct Mips4Cp1ExceptionDecision {
    ...
    /// Sticky IEEE exception flag bits after this operation.
    pub flag_flags: FloatExceptionFlags,
}

/// Decides a floating-point exception update for IEEE cause flags in precise
/// exception mode without mutating the FCSR.
pub const fn decide_float_exception(
    fcsr: Mips4Cp1Fcsr,
    cause: FloatExceptionFlags,
) -> Mips4Cp1ExceptionDecision {
    let traps = fcsr.enable_flags().bits() & cause.bits() != 0;
    let flag_flags = if traps {
        fcsr.flag_flags()
    } else {
        fcsr.flag_flags().union(cause)
    };
    Mips4Cp1ExceptionDecision { traps, flag_flags }
}

/// Decides an Unimplemented Operation exception update in precise exception mode
/// without mutating the FCSR.
///
/// The Unimplemented Operation exception has no enable bit and always traps, and
/// there is no sticky flag bit for it, so the existing flag bits are unchanged.
pub const fn decide_unimplemented_operation(fcsr: Mips4Cp1Fcsr) -> Mips4Cp1ExceptionDecision {
    Mips4Cp1ExceptionDecision {
        traps: true,
        flag_flags: fcsr.flag_flags(),
    }
}
