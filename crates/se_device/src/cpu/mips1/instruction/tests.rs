...
fn extracts_r_format_fields() {
    let instruction =
        Mips1Instruction::from_bits((1 << 21) | (2 << 16) | (3 << 11) | (4 << 6) | 0x20);
...
fn extracts_i_format_fields_and_sign_extends_immediate() {
    let instruction = Mips1Instruction::from_bits((0x08 << 26) | (2 << 21) | (3 << 16) | 0xfffc);
...
fn extracts_j_format_target_and_shifted_index() {
    let instruction = Mips1Instruction::from_bits((0x02 << 26) | 0x0012_3456);
...
fn branch_offset_uses_signed_immediate_shifted_by_two() {
    let instruction = Mips1Instruction::from_bits((0x04 << 26) | 0xfffe);
