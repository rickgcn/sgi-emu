...
fn alternating_instances_replace_same_thread_transaction_state() {
    ...
    assert!(upward.rounding.precision_inexact);
    ...
    assert!(inexact_overflow.rounding.precision_inexact);
...
#[test]
fn synchronized_threads_keep_rounding_flags_and_facts_isolated() {
    ...
    let (positive_halfway, inexact_overflow) = first.join().expect("first worker completed");
    let (negative_halfway, comparisons) = second.join().expect("second worker completed");
