//! Version-one runtime snapshot framing and component dispatch contracts.
...
/// Snapshot container schema version accepted by this crate.
pub const CONTAINER_VERSION: u32 = 1;

/// Maximum encoded length of a stable component key.
...
/// Errors produced while validating a stable component key.
...
/// Stable profile-defined ASCII path identifying one snapshot component.
...
/// Reproducible identity of the exact application build and profile features.
#[repr(transparent)]
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
pub struct BuildFingerprint([u8; 32]);

impl BuildFingerprint {
    /// Creates a build fingerprint from its externally generated bytes.
    #[must_use]
...
/// Reproducible identity of machine topology and guest-visible configuration.
#[repr(transparent)]
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
...
/// One entry in a machine's canonical snapshot component manifest.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SnapshotComponent {
    /// Stable profile-defined component key.
    pub key: ComponentKey,
    /// Component schema version, fixed to one by the current contract.
    pub schema_version: u32,
    /// Maximum accepted Postcard payload length.
    pub max_payload_len: u64,
}

/// Errors produced while constructing or validating a component manifest.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Builder that derives component versions directly from bound targets.
#[derive(Default)]
...
impl SnapshotManifestBuilder {
    ...
    /// Sorts by key bytes and rejects duplicate or unsupported entries.
...
/// Object-safe component dispatch surface implemented by every machine.
pub trait SnapshotTarget {
    /// Returns a sorted, unique, stable component manifest.
...
/// Errors produced by snapshot framing, compatibility checks, or component state.
#[derive(Debug)]
pub enum SnapshotError {
    ...
    TrailingData(u64),
    /// The BLAKE3 integrity value does not match the framed contents.
    IntegrityMismatch,
