...
//! per-CPU delivery and burst truncation. [`address`], [`bus`], and [`decode`]
//! translate CPU and DMA transactions into device-local accesses without coupling
//! device identity to address mappings, and [`device`] joins MMIO, events, state,
//! and introspection for registered devices. [`save`] and [`snapshot`] preserve
//! private component state, and [`inspect`] and [`machine`] provide object-safe
//! runtime surfaces.
//!
